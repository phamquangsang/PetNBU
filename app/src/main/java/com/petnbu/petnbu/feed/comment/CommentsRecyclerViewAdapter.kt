        ...
        private fun displayContent() {
            comment.owner?.run {
                ...
                contentBuilder.apply {
                    ...
                    setSpan(StyleSpan(Typeface.BOLD), start, contentBuilder.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
                    setSpan(ForegroundColorSpan(Color.BLACK), start, contentBuilder.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
                    append("  ")

                    start = contentBuilder.length
                    append(comment.content)
                    setSpan(ForegroundColorSpan(ColorUtils.modifyAlpha(Color.BLACK, 0.8f)), start, contentBuilder.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
                }
