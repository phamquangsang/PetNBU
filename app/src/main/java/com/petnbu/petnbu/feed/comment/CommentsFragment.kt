...
class CommentsFragment : Fragment() {
    ...
    private fun initialize() {
        activity?.run {
            ...
            adapter = CommentsRecyclerViewAdapter(this, feedId, commentsViewModel)
        }
        commentsViewModel.loadComments(feedId).observe(this, Observer { comments -> adapter.submitList(comments) })
        commentsViewModel.commentLoadMoreState.observe(this, Observer { loadMoreState ->
            loadMoreState?.run {
                mBinding.rvComments.post { adapter.addLoadMore = isRunning }
        ...
        mBinding.rvComments.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                ...
                (recyclerView.layoutManager as? LinearLayoutManager)?.run {
                    if (findLastVisibleItemPosition() >= adapter.itemCount - 2 && adapter.itemCount > 0)
