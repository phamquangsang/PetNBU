...
import com.petnbu.petnbu.util.PagingRequestHelper.Request.Callback
import java.util.*
...
/**
 * A helper class for [BoundaryCallback][android.arch.paging.PagedList.BoundaryCallback]s and
...
(private val mRetryService: Executor) {
    ...
    fun runIfNotRunning(type: RequestType, request: Request): Boolean {
        ...
        report?.let { dispatchReport(type, it) }
    ...
    internal fun recordResult(wrapper: RequestWrapper, throwable: Throwable?) {
        ...
        report?.let { dispatchReport(wrapper.mType, it) }
    }

    private fun dispatchReport(requestType: RequestType, report: StatusReport) {
        for (listener in mListeners) {
            listener.onStatusChange(requestType, report)
        }
    }
    ...
    internal class RequestWrapper(val mRequest: Request, val mHelper: PagingRequestHelper,
    ...
    interface Listener {
         ...
         */
        fun onStatusChange(requestType: RequestType, report: StatusReport)
    }
    ...
    enum class RequestType {
         ...
         * a [BoundaryCallback][android.arch.paging.PagedList.BoundaryCallback].
         */
        INITIAL,
        /**
         * Corresponds to the `loadBefore` calls in [DataSource] or
         * `onItemAtFrontLoaded` in
         * [BoundaryCallback][android.arch.paging.PagedList.BoundaryCallback].
         */
        BEFORE,
        /**
...
fun PagingRequestHelper.createStatusLiveData(requestType: RequestType): LiveData<NetworkState> {
    val liveData = MutableLiveData<NetworkState>()
    addListener(object : Listener {
        override fun onStatusChange(rt: RequestType, report: StatusReport) {
            if (requestType == rt) {
                when {
                    report.hasRunning() -> liveData.postValue(NetworkState.LOADING)
                    report.hasError() -> liveData.postValue(NetworkState.error(report.getErrorFor(requestType)?.message))
